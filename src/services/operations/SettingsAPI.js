...
export function updateProfile(token, formData) {
    return async (dispatch) => {
      const toastId = toast.loading("Loading...")
      try {
        const response = await axiosConnector("PUT", UPDATE_PROFILE_API, formData, {
          Authorization: `Bearer ${token}`,
        })
        ...
        // Ensure updatedUserDetails exists before accessing its properties
        ...
        if (updatedUserDetails) {
          ...
          throw new Error("Updated user details not found");
        }
      } catch (error) {
...
export async function changePassword(token, formData) {
  const toastId = toast.loading("Loading...")
  try {
    const response = await axiosConnector("POST", CHANGE_PASSWORD_API, formData, {
      Authorization: `Bearer ${token}`,
    })
...
export function deleteProfile(token, navigate) {
  return async (dispatch) => {
    const toastId = toast.loading("Loading...")
    try {
      const response = await axiosConnector("DELETE", DELETE_PROFILE_API, null, {
        Authorization: `Bearer ${token}`,
      })
