...
export function sendOtp(email, navigate) {
  return async (dispatch) => {
    ...
    try {
      const response = await axiosConnector("POST", SENDOTP_API, {
        email,
      ...
      console.log("If otp send why this printing")
    }
...
export function signUp(accountType, firstName, lastName, email, password, confirmPassword, otp, navigate) {
  ...
  return async (dispatch) => {
    ...
    try {
      ...
      navigate('/login')
      return "success"; // Return success or failure to trigger navigation in the component
    } catch (error) {
      console.log("SIGNUP API ERROR............", error);
      toast.error("Signup Failed");
      toast.error(error?.response?.data?.message)
      return "error"; // Return failure to trigger navigation in the component
    } finally {
      dispatch(setLoading(false));
      toast.dismiss(toastId);
    }
